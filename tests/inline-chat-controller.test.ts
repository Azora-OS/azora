import { AzoraInlineChatController, ChatParticipant, InlineChatOptions } from '../azstudio/src/vs/workbench/contrib/inlineChat/azoraInlineChatController';
import { getChatSessionsService } from '../azstudio/src/vs/workbench/services/chat/chatSessionsService';

describe('AzoraInlineChatController streaming & multi-agent', () => {
  ...
  it('should list participants from registry', () => {
    ...
    expect(participants.some((p: any) => p.id === 'azora.elara')).toBe(true);
  });
  ...
  it('should add custom participants', () => {
    ...
    const participants = controller.getParticipants();
    expect(participants.some((p: any) => p.id === 'custom.agent')).toBe(true);
  });
