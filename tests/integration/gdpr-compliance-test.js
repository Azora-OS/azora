...
async function testDataSubjectRights(results) {
  ...
  try {
    ...
    assert(
      accessResult.subjectId === subjectId,
      'Right of Access should return subject data'
    );
    assert(
      accessResult.data.name === 'John Doe',
      'Access should include subject details'
    );
  ...
  try {
    ...
    const result = await DataSubjectRights.rightToRectification(
      subjectId,
      corrections
    );
    ...
    assert(
      ...
      'Rectification should update subject data'
    );
    assert(
      updated.email === 'jane.smith@example.com',
      'Rectification should update all fields'
    );
  ...
  try {
    ...
    const result = await DataSubjectRights.rightToErasure(
      subjectId,
      'User requested deletion'
    );
    assert(
      !GDPR_DATA.dataSubjects.has(subjectId),
      'Erasure should remove subject data'
    );
...
async function testConsentManagement(results) {
  ...
  try {
    ...
    assert(
      GDPR_DATA.consents.has(result.consentId),
      'Consent should be stored'
    );

    const stored = GDPR_DATA.consents.get(result.consentId);
    assert(
      stored.purposes.includes('marketing'),
      'Consent should include specified purposes'
    );
...
async function testProcessingRecords(results) {
  ...
  try {
    ...
    assert(
      GDPR_DATA.processingRecords.has(result.recordId),
      'Record should be stored'
    );

    results.tests.push({
      name: 'Processing Record Creation',
      status: 'passed',
    });
    results.passed++;
  } catch (error) {
...
async function testBreachNotification(results) {
  ...
  try {
    ...
    assert(
      ...
      'High-risk breach should require notification'
    );

    const breach = GDPR_DATA.breachNotifications.find(
      b => b.breachId === result.breachId
    );
    assert(
      breach.riskAssessment.level === 'high',
...
async function testDPIA(results) {
  ...
  try {
    ...
    assert(
      ...
      'Project should be assessed as high risk'
    );
