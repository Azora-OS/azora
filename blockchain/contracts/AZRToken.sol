...
contract AZRToken is ERC20, ERC20Burnable, Pausable, Ownable {
    uint256 public constant MAX_SUPPLY = 200000000 * 10**18; // 200 million AZR (capped)
    uint256 public constant INITIAL_SUPPLY = 20000000 * 10**18; // 20 million AZR - initial seed
    ...
    event Allocation(address indexed to, uint256 amount, string reason);
    ...
    // Batch allocation to multiple accounts (useful for per-country allocation)
    function batchAllocate(address[] memory accounts, uint256[] memory amounts, string memory reason) external onlyOwner {
        ...
        for (uint256 i = 0; i < accounts.length; i++) {
            ...
            emit Allocation(accounts[i], amounts[i], reason);
        }
    }

    // Allocate tokens to a liquidity pool address (owner sets the pool first)
    ...
    function allocateToLiquidityPool(uint256 amount, string memory reason) external onlyOwner {
        require(liquidityPool != address(0), "Liquidity pool not set");
        require(totalSupply() + amount <= MAX_SUPPLY, "Max supply exceeded");
        _mint(liquidityPool, amount);
        emit Allocation(liquidityPool, amount, reason);
    }
