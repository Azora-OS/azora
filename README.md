...
[![Ubuntu](https://img.shields.io/badge/Ubuntu-Philosophy-orange.svg)](#ubuntu-integration)

> **Current State (Dec 3, 2025)**: **Active Development / Production Preparation**. Core services are implemented with functional blockchain integration, payment processing, and education platform. Advanced features (full Constitutional AI, complete service mesh, mobile apps) are in progress. See `docs/MASTER-TASKLIST-FOR-AGENTS.md` for current priorities and `docs/LAUNCH-CHECKLIST.md` for production readiness status.

### 🚦 Quick Status Snapshot
| Component | Status | Notes |
|---|---|---|
| Blockchain | ✅ Core Implemented | `blockchain/contracts/` - AZR token, CitadelFund, governance deployed; service integration active
| Mint & NFT | ⚠️ Partial | `services/azora-mint/` - Core endpoints; metadata handling in progress
| Pay Service | ⚠️ Partial | `services/azora-pay/` - Integration with Stripe and blockchain underway
| CitadelFund | ✅ Implemented | `services/citadel-fund/` - Core functions; governance audit trail in progress
| Proof-of-Value | ✅ Core Implemented | `services/proof-of-value/` - Mining engine operational; full anti-gaming measures in progress
| Constitutional AI | ⚠️ Partial | `services/constitutional-ai/` - Base rules implemented; self-critique module in progress
| Agent Execution | ✅ Implemented | `services/agent-execution/` - Runtime, sandboxing, analytics operational
| Knowledge Ocean | ⚠️ Partial | `services/knowledge-ocean/` - Indexing operational; pgvector migration pending
| Frontend Apps | ✅ Core Implemented | `apps/azora-sapiens/` and others functional; mobile builds in progress

**Documentation**:
- **Current Tasks**: `docs/MASTER-TASKLIST-FOR-AGENTS.md` - Prioritized task list for development
- **Launch Checklist**: `docs/LAUNCH-CHECKLIST.md` - Production readiness verification
- **Architecture**: `MASTER-IMPLEMENTATION-PLAN.md` - High-level blueprint (reference)
- **Agent Architecture**: `TASKS-AGENT-ARCHITECTURE.md` - Agent system implementation tasks
...
**Current Implementation Status**: Core platform functional with active development on advanced features. Key production tasks include:
- Security hardening and secret management (in progress)
- Complete mobile app releases and CI pipeline
- Full Constitutional AI self-critique and fairness modules
- Service mesh with distributed tracing
- Complete blockchain provider integrations

**Launch Preparation**: See prioritized task list in `docs/MASTER-TASKLIST-FOR-AGENTS.md`
**Verification**: Use `docs/LAUNCH-CHECKLIST.md` for production readiness checks
---
...
## Migration Scripts (azstudio Archive -> azstudio Workbench)

We provide basic helper scripts to safely migrate the Electron-based AzStudio features into the AzStudio Workbench fork (`azstudio`, formerly `azstudio-vscode`):

- `npm run migrate:azstudio` — copies `azstudio.old/src/main/services` (archived source) into `azstudio/src/vs/workbench/contrib/azstudio` (safe, non-destructive copy). If `azstudio.old` isn't present, it will copy from `azstudio` to `azstudio/src/vs/workbench/contrib/azstudio`.
- `npm run archive:azstudio` — rename `azstudio` to `azstudio.old` (destructive; use only after validating `azstudio` is ready)
- `npm run rename:vscode-to-azstudio` — rename `azstudio-vscode` to `azstudio`. Use only after archiving or backing up the original `azstudio`.

See `scripts/azstudio-migration/PLAN.md` for migration phases, checklist, and next steps.
...
We provide quick setup scripts for Windows and Unix-based systems to bootstrap, compile, and run dev smoke tests. These scripts are opinionated convenience helpers for local development and CI.
...
You can also use the `npm` wrapper (cross-shell convenience) from the repo root:
...
If you are running in CI or build servers, prefer the `--skip-native` path and run the full pipeline on the official build runner to validate native builds.
...
- [API Reference](docs/API-Complete.md) - For developers
- [Azora Constitution](docs/AZORA-CONSTITUTION.md) - Our constitutional framework
