...
import { GitService, GitConfig, GitCredentials } from './services/backup/GitService';
import { SecretsVault } from './services/backup/SecretsVault';
...
class AzStudioApp {
  ...
  private setupIPC(): void {
    ...
    // AI operations
    ipcMain.handle('ai:generateCode', async (_event, prompt: string, context: any, model?: any) => {
      try {
        const response = await this.aiOrchestrator.generateCode(prompt, context, model);
        return { success: true, response };
      } catch (error) {
        return { success: false, error: (error as Error).message };
      }
    ...
    ipcMain.handle('ai:estimateCost', async (_event, prompt: string, context: any) => {
      try {
        const cost = await this.aiOrchestrator.estimateCost(prompt, context);
