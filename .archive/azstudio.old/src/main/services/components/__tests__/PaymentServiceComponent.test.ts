...
describe('PaymentServiceComponent', () => {
  ...
  describe('generatePaymentService', () => {
    it('should generate basic payment service files', () => {
      ...
      const filePaths = changes.map(c => c.path.replace(/\\/g, '/'));
      expect(filePaths.some(p => p.includes('index.ts'))).toBe(true);
      expect(filePaths.some(p => p.includes('payment.controller.ts'))).toBe(true);
      expect(filePaths.some(p => p.includes('webhook.controller.ts'))).toBe(true);
      expect(filePaths.some(p => p.includes('stripe.service.ts'))).toBe(true);
      expect(filePaths.some(p => p.includes('payment.service.ts'))).toBe(true);
      expect(filePaths.some(p => p.includes('payment.repository.ts'))).toBe(true);
      expect(filePaths.some(p => p.includes('schema.prisma'))).toBe(true);
    });
    ...
    it('should include subscription service when enabled', () => {
      ...
      expect(filePaths.some(p => p.includes('subscription.service.ts'))).toBe(true);
    });
