...
export class StakingService {
    ...
    calculateAPY(duration) {
        ...
        if (duration >= 365)
            return 12; // 12% for 1 year
        if (duration >= 180)
            return 8; // 8% for 6 months
        if (duration >= 90)
            return 6; // 6% for 3 months
        if (duration >= 30)
            return 4; // 4% for 1 month
    ...
    async getStakingStats() {
        const [totalStaked, activeStakes, totalStakers, averageAPY] = await Promise.all([
