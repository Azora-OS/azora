...
export class CreditService {
     ...
     */
    async applyForCredit(userId, amount, purpose) {
        try {
     ...
     */
    async analyzeCreditApplication(userId, amount, purpose, trustScore) {
        try {
            const prompt = `
     ...
     */
    async createLoan(applicationId, userId, amount, interestRate, termMonths) {
     ...
     */
    async makeRepayment(userId, loanId, amount) {
    ...
    async getTrustScore(userId) {
        ...
        if (!trustScore || (Date.now() - trustScore.lastUpdated.getTime()) > 24 * 60 * 60 * 1000) {
            ...
            const overall = Math.round((factors.transactionHistory * 0.3) +
     ...
     */
    async calculateTrustFactors(userId) {
