...
export class DefiService {
     ...
     */
    async depositToPool(userId, poolId, amount) {
        try {
            if (amount <= 0) {
    ...
    async getUserPositions(userId) {
        ...
        const positionsWithRewards = await Promise.all(positions.map(async (position) => {
