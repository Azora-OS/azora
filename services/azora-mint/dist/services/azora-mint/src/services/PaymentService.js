...
export class PaymentService {
     ...
     */
    async sendPayment(senderId, recipientId, amount, description) {
        try {
            if (amount <= 0) {
     ...
     */
    async requestPayment(requesterId, recipientId, amount, description) {
        try {
            if (amount <= 0) {
     ...
     */
    async fulfillPaymentRequest(userId, requestId) {
        try {
            ...
            // Send the payment
            const payment = await this.sendPayment(userId, request.requesterId, request.amount, `Payment for: ${request.description}`);
    ...
    async getPaymentStats(userId) {
        const [sentCount, receivedCount, totalSent, totalReceived, pendingRequests] = await Promise.all([
     ...
     */
    async createEscrowPayment(buyerId, sellerId, amount, itemId) {
        try {
     ...
     */
    async releaseEscrowPayment(escrowId, releaserId) {
        try {
