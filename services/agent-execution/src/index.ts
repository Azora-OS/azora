...
app.get('/execution/:id', async (req, res) => {
  try {
    if (!process.env.DATABASE_URL) return res.status(404).json({ error: 'No DB configured' });
    const id = req.params.id;
    const { getExecutionRecord } = require('./repo');
    const e = await getExecutionRecord(id);
...
app.get('/task/:id/executions', async (req, res) => {
  try {
    if (!process.env.DATABASE_URL) return res.status(404).json({ error: 'No DB configured' });
    const id = req.params.id;
    const prisma = require('./prismaClient').default;
    const results = await prisma.execution.findMany({ where: { taskId: id }, orderBy: { createdAt: 'desc' } });
