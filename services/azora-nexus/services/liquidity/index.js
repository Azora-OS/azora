...
const swaggerOptions = {
  definition: {
    openapi: '3.0.0',
    info: {
      title: 'Azora Nexus Liquidity Service API',
      version: '1.0.0',
      description: 'API documentation for liquidity service',
    },
    servers: [
      {
        url: `http://localhost:${process.env.PORT || 4112}`,
...
app.get('/health', (req, res) => { res.json({ status: 'healthy', service: 'liquidity', timestamp: new Date().toISOString(), uptime: process.uptime() }) })

app.post('/api/liquidity/add', (req, res) => { logger.info('Add liquidity to pool', { body: req.body }); res.json({ poolId: '', message: 'Liquidity service endpoint - implementation pending' }) })
...
app.listen(PORT, HOST, () => { logger.info(`💧 Azora Nexus Liquidity Service running on ${HOST}:${PORT}`, { environment: process.env.NODE_ENV || 'development', service: 'liquidity' }) })
