...
const swaggerOptions = {
  definition: {
    openapi: '3.0.0',
    info: {
      title: 'Azora Nexus Global Adoption Service API',
      version: '1.0.0',
      description: 'API documentation for global-adoption service',
    },
    servers: [
      {
        url: `http://localhost:${process.env.PORT || 4108}`,
...
app.get('/health', (req, res) => { res.json({ status: 'healthy', service: 'global-adoption', timestamp: new Date().toISOString(), uptime: process.uptime() }) })

app.get('/api/global-adoption/metrics', (req, res) => { logger.info('Get adoption metrics'); res.json({ metrics: {}, regions: [], message: 'Global Adoption service endpoint - implementation pending' }) })
...
app.listen(PORT, HOST, () => { logger.info(`🌍 Azora Nexus Global Adoption Service running on ${HOST}:${PORT}`, { environment: process.env.NODE_ENV || 'development', service: 'global-adoption' }) })
