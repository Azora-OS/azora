...
const swaggerOptions = {
  definition: {
    openapi: '3.0.0',
    info: {
      title: 'Azora Nexus Guardian Service API',
      version: '1.0.0',
      description: 'API documentation for guardian service',
    },
    servers: [
      {
        url: `http://localhost:${process.env.PORT || 4109}`,
...
app.get('/health', (req, res) => { res.json({ status: 'healthy', service: 'guardian', timestamp: new Date().toISOString(), uptime: process.uptime() }) })

app.post('/api/guardian/protect', (req, res) => { logger.info('Enable asset protection', { body: req.body }); res.json({ protected: true, message: 'Guardian service endpoint - implementation pending' }) })
...
app.listen(PORT, HOST, () => { logger.info(`🛡️ Azora Nexus Guardian Service running on ${HOST}:${PORT}`, { environment: process.env.NODE_ENV || 'development', service: 'guardian' }) })
