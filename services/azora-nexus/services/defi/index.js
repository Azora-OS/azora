...
const swaggerOptions = {
  definition: {
    openapi: '3.0.0',
    info: {
      title: 'Azora Nexus DeFi Service API',
      version: '1.0.0',
      description: 'API documentation for defi service',
    },
    servers: [
      {
        url: `http://localhost:${process.env.PORT || 4107}`,
...
app.get('/health', (req, res) => { res.json({ status: 'healthy', service: 'defi', timestamp: new Date().toISOString(), uptime: process.uptime() }) })

app.post('/api/defi/liquidity/add', (req, res) => { logger.info('Add liquidity to pool', { body: req.body }); res.json({ poolId: '', message: 'DeFi service endpoint - implementation pending' }) })
...
app.listen(PORT, HOST, () => { logger.info(`🌊 Azora Nexus DeFi Service running on ${HOST}:${PORT}`, { environment: process.env.NODE_ENV || 'development', service: 'defi' }) })
