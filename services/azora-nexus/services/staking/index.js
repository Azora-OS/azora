...
const swaggerOptions = {
  definition: {
    openapi: '3.0.0',
    info: {
      title: 'Azora Nexus Staking Service API',
      version: '1.0.0',
      description: 'API documentation for staking service',
    },
    servers: [
      {
        url: `http://localhost:${process.env.PORT || 4118}`,
...
app.get('/health', (req, res) => { res.json({ status: 'healthy', service: 'staking', timestamp: new Date().toISOString(), uptime: process.uptime() }) })

app.post('/api/staking/stake', (req, res) => { logger.info('Stake tokens', { body: req.body }); res.json({ staked: true, message: 'Staking service endpoint - implementation pending' }) })
...
app.listen(PORT, HOST, () => { logger.info(`🔒 Azora Nexus Staking Service running on ${HOST}:${PORT}`, { environment: process.env.NODE_ENV || 'development', service: 'staking' }) })
