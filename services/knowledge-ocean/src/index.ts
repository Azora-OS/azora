...
app.get('/graph/:id/related', async (req, res) => {
  const depth = parseInt(req.query.depth as string) || 2;
  const related = graph.getRelated(req.params.id, depth);
  res.json({ related });
});

app.get('/graph/:id/versions', async (req, res) => {
  const versions = graph.getVersionHistory(req.params.id);
  res.json({ versions });
});

app.get('/health', (req, res) => {
  res.json({ status: 'healthy', service: 'knowledge-ocean' });
});
