...
export class HierarchicalRouter implements IHierarchicalRouter {
  ...
  private knowledgeOcean: any; // Using any for now to avoid circular dependency issues
  ...
  constructor(
    ...
    // Initialize Knowledge Ocean
    const { knowledgeOcean } = require('@azora/shared-api');
  ...
  private async processWithTier(query: AIQuery, tier: RoutingTier): Promise<AIResponse> {
    ...
    // Check cache first
    const queryHash = this.hashQuery(query.query);
    // 2. Check Knowledge Ocean (Static & Predicted)
    const knowledgeResult = this.knowledgeOcean.tryAnswer(query);
    if (knowledgeResult) {
      // Check if it's a predicted entry
      const prediction = this.knowledgeOcean.getPrediction(query);

      return {
        id: `response-${Date.now()}`, // Assuming AIResponse needs an ID
        content: knowledgeResult,
        routingTier: RoutingTier.LOCAL_LLM, // Or a new 'KNOWLEDGE_OCEAN' tier
        responseTime: Date.now() - startTime,
        cost: 0,
        cached: true,
        metadata: {
          model: 'knowledge-ocean',
          provider: 'azora-local',
          latency: Date.now() - startTime,
          cost: 0,
          tokens: 0,
          tier: 'local',
          cached: true,
          prpengUsed: !!prediction,
          predictionSource: prediction?.source || 'static',
          predictionConfidence: prediction?.confidence
        }
      };
    }

    // 3. PrPEng Prediction Generation (Async/Background)
    // For complex queries that miss cache, we trigger prediction generation
    // This happens in parallel or before external call depending on strategy
    // For MVP, we'll assume we proceed to external call but log the intent
    const prpengEnabled = true; // Should come from config
