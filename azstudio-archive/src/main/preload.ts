...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  // Chat operations (Phase C)
  chat: {
    createSession: (agentId: string, userContext?: any) =>
      ipcRenderer.invoke('chat:createSession', agentId, userContext),
    sendMessage: (sessionId: string, message: string) =>
      ipcRenderer.invoke('chat:sendMessage', sessionId, message),
    sendMessageStreaming: (sessionId: string, message: string) =>
      ipcRenderer.invoke('chat:sendMessageStreaming', sessionId, message),
    getSession: (sessionId: string) =>
      ipcRenderer.invoke('chat:getSession', sessionId),
    listSessions: () =>
      ipcRenderer.invoke('chat:listSessions'),
    archiveSession: (sessionId: string) =>
      ipcRenderer.invoke('chat:archiveSession', sessionId),
    updateSessionContext: (sessionId: string, context: any) =>
      ipcRenderer.invoke('chat:updateSessionContext', sessionId, context),
  },

  // Orchestrator operations (Phase E)
  orchestrator: {
    planTask: (userPrompt: string, context: any) =>
      ipcRenderer.invoke('orchestrator:planTask', userPrompt, context),
    executeTask: (taskDAG: any, projectRoot: string) =>
      ipcRenderer.invoke('orchestrator:executeTask', taskDAG, projectRoot),
    verifyCode: (code: string, filePath: string) =>
      ipcRenderer.invoke('orchestrator:verifyCode', code, filePath),
    applyChanges: (changeset: any, projectRoot: string) =>
      ipcRenderer.invoke('orchestrator:applyChanges', changeset, projectRoot),
    rollback: (changesetId: string, projectRoot: string) =>
      ipcRenderer.invoke('orchestrator:rollback', changesetId, projectRoot),
  },

  // Security operations (Phase F)
  security: {
    checkLicense: (workspacePath: string) =>
      ipcRenderer.invoke('security:checkLicense', workspacePath),
    getFeatures: (workspacePath: string) =>
      ipcRenderer.invoke('security:getFeatures', workspacePath),
    setSecret: (key: string, value: string, scope: 'global' | 'project') =>
      ipcRenderer.invoke('security:setSecret', key, value, scope),
    getSecret: (key: string, scope: 'global' | 'project') =>
      ipcRenderer.invoke('security:getSecret', key, scope),
    requestPermission: (options: any) =>
      ipcRenderer.invoke('security:requestPermission', options),
    hasPermission: (type: string, resource: string) =>
      ipcRenderer.invoke('security:hasPermission', type, resource),
    getAuditLog: (options?: any) =>
      ipcRenderer.invoke('security:getAuditLog', options),
    addToAllowlist: (domain: string) =>
      ipcRenderer.invoke('security:addToAllowlist', domain),
    removeFromAllowlist: (domain: string) =>
      ipcRenderer.invoke('security:removeFromAllowlist', domain),
  },

  // Testing operations (Phase G)
  testing: {
    runE2ETests: (scenarios?: any[]) =>
      ipcRenderer.invoke('testing:runE2ETests', scenarios),
    runPerformanceTests: (options?: any) =>
      ipcRenderer.invoke('testing:runPerformanceTests', options),
    auditPerformance: (url: string, options?: any) =>
      ipcRenderer.invoke('testing:auditPerformance', url, options),
    checkAccessibility: (url: string, options?: any) =>
      ipcRenderer.invoke('testing:checkAccessibility', url, options),
    getTestResults: (testType: 'e2e' | 'performance' | 'accessibility') =>
      ipcRenderer.invoke('testing:getTestResults', testType),
  },
  ...
  onChatChunk: (callback: (data: { sessionId: string; chunk: string; metadata?: any }) => void) => {
...
export interface ElectronAPI {
  ...
  project: {
    ...
    detectFrameworks: (rootPath: string) => Promise<{ success: boolean; frameworks?: any; error?: string }>;
    getConventions: (rootPath: string) => Promise<{ success: boolean; conventions?: any; error?: string }>;
    detectStructure: (rootPath: string) => Promise<{ success: boolean; structure?: any; error?: string }>;
  };
  network: {
  ...
  chat: {
    createSession: (agentId: string, userContext?: any) => Promise<{ success: boolean; session?: any; error?: string }>;
    sendMessage: (sessionId: string, message: string) => Promise<{ success: boolean; response?: any; error?: string }>;
    sendMessageStreaming: (sessionId: string, message: string) => Promise<{ success: boolean; response?: any; error?: string }>;
    ...
    updateSessionContext: (sessionId: string, context: any) => Promise<{ success: boolean; error?: string }>;
  };
  orchestrator: {
    planTask: (userPrompt: string, context: any) => Promise<{ success: boolean; taskDAG?: any; error?: string }>;
    executeTask: (taskDAG: any, projectRoot: string) => Promise<{ success: boolean; results?: any[]; error?: string }>;
    verifyCode: (code: string, filePath: string) => Promise<{ success: boolean; pipeline?: any; gate?: any; error?: string }>;
    applyChanges: (changeset: any, projectRoot: string) => Promise<{ success: boolean; result?: any; error?: string }>;
    rollback: (changesetId: string, projectRoot: string) => Promise<{ success: boolean; result?: any; error?: string }>;
  };
  security: {
    checkLicense: (workspacePath: string) => Promise<{ success: boolean; licenseType?: string; error?: string }>;
    getFeatures: (workspacePath: string) => Promise<{ success: boolean; features?: any; error?: string }>;
    setSecret: (key: string, value: string, scope: 'global' | 'project') => Promise<{ success: boolean; error?: string }>;
    getSecret: (key: string, scope: 'global' | 'project') => Promise<{ success: boolean; value?: string; error?: string }>;
    requestPermission: (options: any) => Promise<{ success: boolean; granted?: boolean; error?: string }>;
    hasPermission: (type: string, resource: string) => Promise<{ success: boolean; hasPermission?: boolean; error?: string }>;
    getAuditLog: (options?: any) => Promise<{ success: boolean; log?: any[]; error?: string }>;
    addToAllowlist: (domain: string) => Promise<{ success: boolean; error?: string }>;
    removeFromAllowlist: (domain: string) => Promise<{ success: boolean; error?: string }>;
  };

  // Testing operations (Phase G)
  testing: {
    runE2ETests: (scenarios?: any[]) => Promise<{ success: boolean; results?: any; error?: string }>;
    runPerformanceTests: (options?: any) => Promise<{ success: boolean; results?: any; error?: string }>;
    auditPerformance: (url: string, options?: any) => Promise<{ success: boolean; report?: any; error?: string }>;
    checkAccessibility: (url: string, options?: any) => Promise<{ success: boolean; report?: any; error?: string }>;
    getTestResults: (testType: 'e2e' | 'performance' | 'accessibility') => Promise<{ success: boolean; results?: any; error?: string }>;
  };
  onUpdateAvailable: (callback: () => void) => void;
  onUpdateDownloaded: (callback: () => void) => void;
  onUpdateError: (callback: (error: string) => void) => void;
  onFileChanged: (callback: (event: any) => void) => void;
  onChatChunk: (callback: (data: { sessionId: string; chunk: string; metadata?: any }) => void) => void;
}
