...
export class ElaraAgenticService {
  ...
  // Streaming variant of constitutional check generator.
  async *generateWithConstitutionalCheckStreaming(
    prompt: string,
    systemRole: string,
  ): AsyncGenerator<string, void, void> {
    // Attempt to use AIOrchestrator streaming if available
    try {
      const AIOrch = require('./AIOrchestrator').AIOrchestrator;
      const orchestrator = new AIOrch();
      const ctx = { files: [], projectInfo: { frameworks: [], conventions: {} }, userPrompt: prompt };
      for await (const part of orchestrator.generateCodeStreaming(prompt, ctx)) {
        yield part.chunk;
      }
    } catch (err) {
      // fallback to non-streaming and yield as a single chunk
      const content = await this.generateWithConstitutionalCheck(prompt, systemRole, 'text');
      yield content;
    }
  }
