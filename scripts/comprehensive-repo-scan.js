...
class ComprehensiveRepoScanner {
  ...
  async generateReport() {
    ...
    if (this.missing.some(item => item.includes('Core service'))) {
      console.log('  2. Create missing core services');
    }
    
    if (this.missing.some(item => item.includes('Core app'))) {
      console.log('  3. Set up missing frontend apps');
    }
    ...
    console.log('  5. Run: node scripts/connect-frontends-backends.js');
    console.log('  6. Test: node scripts/health-check.js');
    ...
    if (totalIssues === 0) {
      ...
      console.log(`\n🔧 ${totalIssues} issues need to be resolved before launch`);
    }
  }
}
