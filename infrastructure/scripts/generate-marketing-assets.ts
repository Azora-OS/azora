...
/**
 * Azora OS Marketing Assets Generator
...
// Generate SVG Logo
function generateSVGLogo(service: typeof SERVICES[0], size: number = 512): string {
  const { name, shortName, icon, color, tagline } = service;

  // Create circular or square logo with icon and text
  const logoType = 'circular'; // or 'square'

  if (logoType === 'circular') {
    return `
...
function generatePosterHTML(service: typeof SERVICES[0]): string {
...
function generateVideoScript(service: typeof SERVICES[0]): string {
