...
export const AIFamilyChat = ({ initialMember = 'elara', showFamilyMembers = true, onMemberSwitch, }) => {
    ...
    useEffect(() => {
        ...
        const personality = AI_PERSONALITIES[currentMember];
    ...
    const addAIMessage = (content, mood) => {
        const newMessage = {
    ...
    const getAIResponse = (userMessage) => {
        const personality = AI_PERSONALITIES[currentMember];
        if (!personality)
            return { response: "I'm not available right now!" };
    ...
    const switchMember = (memberId) => {
        setCurrentMember(memberId);
        onMemberSwitch?.(memberId);
        // Add transition message
        const personality = AI_PERSONALITIES[memberId];
        if (personality) {
            setTimeout(() => {
                const greeting = personality.greetings[Math.floor(Math.random() * personality.greetings.length)];
                addAIMessage(greeting);
    ...
    const currentPersonality = AI_PERSONALITIES[currentMember];
    return (_jsxs("div", { className: "flex flex-col h-[600px] bg-gradient-to-br from-slate-900 via-purple-900/20 to-slate-900 rounded-2xl border border-purple-500/30 overflow-hidden", children: [_jsxs("div", { className: "flex items-center justify-between px-6 py-4 bg-black/40 backdrop-blur-lg border-b border-purple-500/30", children: [_jsxs("div", { className: "flex items-center gap-3", children: [currentMember === 'elara' && _jsx(ElaraAvatar, { size: 48, animate: true }), currentMember === 'sankofa' && _jsx(SankofaAvatar, { size: 48, animate: true }), !['elara', 'sankofa'].includes(currentMember) && (_jsx("div", { className: "text-4xl", children: currentPersonality?.emoji })), _jsxs("div", { children: [_jsx("h3", { className: "text-lg font-bold text-white", children: currentPersonality?.name }), _jsx("p", { className: "text-xs text-gray-400", children: isTyping ? 'Typing...' : 'Online' })] })] }), showFamilyMembers && (_jsx("div", { className: "flex gap-2", children: Object.entries(AI_PERSONALITIES).slice(0, 6).map(([id, member]) => (_jsx("button", { onClick: () => switchMember(id), className: `text-2xl hover:scale-110 transition-transform ${currentMember === id ? 'scale-125 drop-shadow-lg' : 'opacity-60'}`, title: member.name, children: member.emoji }, id))) }))] }), _jsxs("div", { className: "flex-1 overflow-y-auto px-6 py-4 space-y-4", children: [messages.map(message => (_jsxs("div", { className: `flex gap-3 ${message.sender === 'user' ? 'flex-row-reverse' : 'flex-row'}`, children: [message.sender === 'ai' && (_jsxs("div", { className: "flex-shrink-0", children: [message.aiMember === 'elara' && _jsx(ElaraAvatar, { size: 40, animate: false, mood: message.mood }), message.aiMember === 'sankofa' && _jsx(SankofaAvatar, { size: 40, animate: false, mood: message.mood }), !['elara', 'sankofa'].includes(message.aiMember || '') && (_jsx("div", { className: "w-10 h-10 rounded-full flex items-center justify-center text-2xl bg-black/40", children: AI_PERSONALITIES[message.aiMember]?.emoji }))] })), _jsxs("div", { className: `max-w-[70%] rounded-2xl px-4 py-3 ${message.sender === 'user'
                                    ? 'bg-purple-600 text-white'
                                    : 'bg-black/40 text-gray-100 border border-purple-500/20'}`, children: [_jsx("p", { className: "text-sm whitespace-pre-wrap", children: message.content }), _jsx("span", { className: "text-xs opacity-60 mt-1 block", children: message.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) })] })] }, message.id))), isTyping && (_jsxs("div", { className: "flex gap-3", children: [_jsxs("div", { className: "flex-shrink-0", children: [currentMember === 'elara' && _jsx(ElaraAvatar, { size: 40, animate: true }), currentMember === 'sankofa' && _jsx(SankofaAvatar, { size: 40, animate: true }), !['elara', 'sankofa'].includes(currentMember) && (_jsx("div", { className: "w-10 h-10 rounded-full flex items-center justify-center text-2xl bg-black/40", children: currentPersonality?.emoji }))] }), _jsx("div", { className: "bg-black/40 rounded-2xl px-4 py-3 border border-purple-500/20", children: _jsxs("div", { className: "flex gap-1", children: [_jsx("div", { className: "w-2 h-2 bg-purple-400 rounded-full animate-bounce", style: { animationDelay: '0ms' } }), _jsx("div", { className: "w-2 h-2 bg-purple-400 rounded-full animate-bounce", style: { animationDelay: '150ms' } }), _jsx("div", { className: "w-2 h-2 bg-purple-400 rounded-full animate-bounce", style: { animationDelay: '300ms' } })] }) })] })), _jsx("div", { ref: messagesEndRef })] }), _jsxs("div", { className: "px-6 py-4 bg-black/40 backdrop-blur-lg border-t border-purple-500/30", children: [_jsxs("div", { className: "flex gap-3", children: [_jsx("input", { type: "text", value: input, onChange: (e) => setInput(e.target.value), onKeyPress: (e) => e.key === 'Enter' && handleSendMessage(), placeholder: `Chat with ${currentPersonality?.name}... Try asking about family! 👨‍👩‍👧‍👦`, className: "flex-1 bg-white/5 border border-purple-500/30 rounded-xl px-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:border-purple-500/60" }), _jsx("button", { onClick: handleSendMessage, disabled: !input.trim(), className: "px-6 py-3 bg-purple-600 hover:bg-purple-700 disabled:bg-gray-700 disabled:cursor-not-allowed text-white rounded-xl font-medium transition-colors", children: "Send" })] }), _jsx("p", { className: "text-xs text-gray-400 mt-2 text-center", children: "\uD83D\uDCA1 Try: \"How's your mom?\" \"Tell me about your family!\" \"Any funny stories?\"" })] })] }));
};
